 ...
 * @method     ChildAccountQuery orderByUserId($order = Criteria::ASC) Order by the user_id column
 ...
 * @method     ChildAccountQuery groupByUserId() Group by the user_id column
 ...
 *
 * @method     \App\Models\UserQuery|\App\Models\TransactionQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildAccount findOneByUserId(int $user_id) Return the first ChildAccount filtered by the user_id column
 ...
 * @method     ChildAccount requireOneByUserId(int $user_id) Return the first ChildAccount filtered by the user_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildAccount[]|ObjectCollection findByUserId(int $user_id) Return ChildAccount objects filtered by the user_id column
...
abstract class AccountQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \App\Models\Base\AccountQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\App\\Models\\Account', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, user_id, name FROM account WHERE id = :p0';
        try {
    ...
    public function filterByUserId($userId = null, $comparison = null)
    {
        if (is_array($userId)) {
            $useMinMax = false;
            if (isset($userId['min'])) {
                $this->addUsingAlias(AccountTableMap::COL_USER_ID, $userId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($userId['max'])) {
                $this->addUsingAlias(AccountTableMap::COL_USER_ID, $userId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(AccountTableMap::COL_USER_ID, $userId, $comparison);
    }
    ...
    public function filterByUser($user, $comparison = null)
    {
        if ($user instanceof \App\Models\User) {
            return $this
                ->addUsingAlias(AccountTableMap::COL_USER_ID, $user->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(AccountTableMap::COL_USER_ID, $user->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
    ...
    /**
     * Filter the query by a related \App\Models\Transaction object
     *
     * @param \App\Models\Transaction|ObjectCollection $transaction the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildAccountQuery The current query, for fluid interface
     */
    public function filterByTransaction($transaction, $comparison = null)
    {
        if ($transaction instanceof \App\Models\Transaction) {
            ...
            throw new PropelException('filterByTransaction() only accepts arguments of type \App\Models\Transaction or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \App\Models\TransactionQuery A secondary query class using the current class as primary query
     */
    public function useTransactionQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ...
            ->useQuery($relationAlias ? $relationAlias : 'Transaction', '\App\Models\TransactionQuery');
    }
